...
! !CONSTANTS
   real(RKIND), parameter    :: r80fac = 1.65     ! ratio of radius(RH=0.8)/radius(RH=0.) [Gerber]
   real(RKIND), parameter    :: rhop = 2200.      ! dry seasalt density [kg m-3]
...
! !INPUT PARAMETERS:
   ...
   real(RKIND),    intent(in)           :: rLow, rUp   ! Dry particle bin edge radii [um]
   real(RKIND),    intent(in)           :: w10m        ! 10-m wind speed [m s-1]
   real(RKIND),    intent(in)           :: ustar       ! friction velocity [m s-1]
   real(RKIND),    intent(in)           :: pi          ! ratio of a circle's circumference to its diameter
   integer, intent(in)           :: method      ! Algorithm to use

! !OUTPUT PARAMETERS:

   real(RKIND),    intent(inout)        :: memissions      ! Mass Emissions Flux [kg m-2 s-1]
   real(RKIND),    intent(inout)        :: nemissions      ! Number Emissions Flux [# m-2 s-1]
   integer, intent(out)          :: rc              ! Error return code:
                                                    !  0 - all is well
                                                    !  1 - 
! !Local Variables
   integer       :: ir
   real(RKIND)          :: w                               ! Intermediary wind speed [m s-1]
   real(RKIND)          :: r, dr                           ! sub-bin radius spacing (dry, um)
   real(RKIND)          :: rwet, drwet                     ! sub-bin radius spacing (rh=80%, um)
   real(RKIND)          :: aFac, bFac, scalefac, rpow, exppow, wpow
...
! where r is the particle radius at 80% RH, dr is the size bin width at 80% RH, and w is the wind speed
  ...
  function SeasaltEmissionGong ( r, dr, w, scalefac, aFac, bFac, rpow, exppow, wpow )
   ...
   real(RKIND), intent(in)    :: r, dr     ! Wet particle radius, bin width [um]
   real(RKIND), intent(in)    :: w         ! Grid box mean wind speed [m s-1] (10-m or ustar wind)
   real(RKIND), intent(in)    :: scalefac, aFac, bFac, rpow, exppow, wpow
   real(RKIND)                :: SeasaltEmissionGong
