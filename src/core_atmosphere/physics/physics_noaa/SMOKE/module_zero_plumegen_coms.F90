...
module module_zero_plumegen_coms
  ...
  type plumegen_coms
    ...
    real(kind=RKIND),dimension(nkp) ::  w,t,qv,qc,qh,qi,sc,  &  ! blob
         vth,vti,rho,txs,  &
         est,qsat! never used: ,qpas,qtotal

    real(kind=RKIND),dimension(nkp) ::  wc,wt,tt,qvt,qct,qht,qit,sct
    real(kind=RKIND),dimension(nkp) ::  dzm,dzt,zm,zt,vctr1,vctr2 &
         ,vt3dc,vt3df,vt3dk,vt3dg,scr1

    real(kind=RKIND),dimension(nkp) ::  pke,the,thve,thee,pe,te,qvenv,dne ! environment at plume grid ! never used: rhe, sce
    real(kind=RKIND),dimension(nkp) ::  ucon,vcon,thtcon ,rvcon,picon,tmpcon & ! never used: wcon, dncon, prcon
         ,zcon,zzcon ! environment at RAMS  grid ! never used: scon

    real(kind=RKIND) :: DZ,DQSDZ,VISC(nkp),VISCOSITY,TSTPF   
    integer :: N,NM1,L
    !
    real(kind=RKIND) :: CVH(nkp),CVI(nkp),ADIABAT,&
         WBAR,VHREL,VIREL  ! advection
    ! Never used: ADVW,ADVT,ADVV,ADVC,ADVH,ADVI,ALAST(10)

    !
    real(kind=RKIND) :: ZSURF,ZTOP ! never used: ZBASE
    ! never used: integer :: LBASE
    !
    real(kind=RKIND) :: AREA,RSURF,ALPHA,RADIUS(nkp)  ! entrain
    !
    real(kind=RKIND) :: HEATING(ntime),FMOIST,BLOAD   ! heating
    !
    real(kind=RKIND) :: DT,TIME,TDUR
    integer :: MINTIME,MDUR,MAXTIME
    !
    !REAL(kind=RKIND),DIMENSION(nkp,2)  :: W_VMD,VMD
    REAL(kind=RKIND) :: upe   (nkp)
    REAL(kind=RKIND) :: vpe   (nkp)
    REAL(kind=RKIND) :: vel_e (nkp)

    REAL(kind=RKIND) :: vel_p (nkp)
    REAL(kind=RKIND) :: rad_p (nkp)
    REAL(kind=RKIND) :: vel_t (nkp)
    REAL(kind=RKIND) :: rad_t (nkp)

    REAL(kind=RKIND) :: ztop_(ntime)
    integer :: testval
  contains
